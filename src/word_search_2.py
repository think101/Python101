    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def exist(word: str) -> bool:
            ...
            def dfs(index: int, i: int, j: int) -> bool:
                if i < 0 or i >= row or j < 0 or j >= col or index >= word_len or bd[i][j] != word[index:index + 1]:
                    return False
                ...
                t = bd[i][j]
                bd[i][j] = 0
                result = dfs(index + 1, i - 1, j) or dfs(index + 1, i + 1, j) or dfs(index + 1, i, j - 1) or dfs(
                    index + 1, i, j + 1)
                if not result:
                    bd[i][j] = t
                return result
